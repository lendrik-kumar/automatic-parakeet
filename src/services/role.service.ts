...
export const getRole = async (id: string) => {
  ...
  if (!role) throw new RoleError(404, "Role not found");
  return role;
};

export const createRole = async (adminId: string, data: CreateRoleInput) => {
...
export const updateRole = async (
  ...
  if (!existing) throw new RoleError(404, "Role not found");
...
export const deleteRole = async (adminId: string, id: string) => {
  const existing = await roleRepository.findById(id);
  if (!existing) throw new RoleError(404, "Role not found");
  ...
  if (adminCount > 0) {
    throw new RoleError(
      400,
      `Cannot delete role with ${adminCount} admins assigned. Please reassign or delete admins first.`,
    );
  }
...
export const getRoleAdmins = async (id: string) => {
  const role = await roleRepository.findById(id);
  if (!role) throw new RoleError(404, "Role not found");
