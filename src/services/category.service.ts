...
export const createCategory = async (
  adminId: string,
  data: Omit<CreateCategoryInput, "slug"> & { slug?: string },
) => {
...
export const deleteCategory = async (adminId: string, id: string) => {
  const existing = await categoryRepository.findById(id);
  if (!existing) throw new CategoryError(404, "Category not found");
  ...
  if (existing._count.subcategories > 0) {
    throw new CategoryError(
      400,
      `Cannot delete category with ${existing._count.subcategories} subcategories. Please delete or reassign subcategories first.`,
    );
  }
...
export const reorderCategories = async (
  ...
  const categories = await Promise.all(
    orderMap.map((item) => categoryRepository.findById(item.id)),
  );
...
export const getCategorySubcategories = async (id: string) => {
