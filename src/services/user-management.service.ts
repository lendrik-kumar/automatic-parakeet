...
export const updateUserProfile = async (
  ...
  const updated = await userRepository.update(userId, data as never);
  await adminRepository.logActivity(adminId, "UPDATE", "User", userId);
  return updated;
};
...
export const resetUserPassword = async (adminId: string, userId: string) => {
  ...
  return {
    userId,
    message:
      "Password reset requested. Integrate email/SMS flow to deliver reset link.",
  };
};

export const getUserOrders = async (userId: string, page = 1, limit = 20) => {
  const user = await prisma.user.findUnique({ where: { id: userId } });
  if (!user) throw new UserManagementError(404, "User not found");
...
export const getUserReviews = async (userId: string, page = 1, limit = 20) => {
  const user = await prisma.user.findUnique({ where: { id: userId } });
  if (!user) throw new UserManagementError(404, "User not found");
...
export const getUserActivity = async (userId: string, page = 1, limit = 20) => {
  const user = await prisma.user.findUnique({ where: { id: userId } });
  if (!user) throw new UserManagementError(404, "User not found");
...
export const bulkUpdateUserStatus = async (
  ...
  if (!userIds.length) throw new UserManagementError(400, "User IDs are required");
...
export const getUserLifetimeValue = async (userId: string) => {
  const user = await prisma.user.findUnique({ where: { id: userId } });
  if (!user) throw new UserManagementError(404, "User not found");
