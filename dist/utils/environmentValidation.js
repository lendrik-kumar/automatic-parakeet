...
export const validateEnvironment = () => {
    ...
    for (const config of ENV_VARIABLES) {
        ...
        if (isRequired && (!value || value.trim().length === 0)) {
            errors.push(`Missing required environment variable: ${config.variable} (${config.description})`);
            continue;
        }
        ...
        if (!validateValue(config, value)) {
            errors.push(`Invalid format for ${config.variable}: ${config.description}${config.format ? ` (expected: ${config.format})` : ""}`);
        }
    }
    return errors;
};
...
export const getEnvironmentSummary = () => {
    ...
    for (const config of ENV_VARIABLES) {
        ...
        if (value) {
            summary.configured[config.variable] =
                config.format === "api_key" || config.variable.includes("SECRET")
                    ? `${value.slice(0, 4)}...${value.slice(-4)}`
                    : value;
        }
        else if (isRequired) {
            summary.missing.push(config.variable);
        }
    }
    return summary;
};
