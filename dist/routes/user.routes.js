...
import { initiatePhoneRegistration, resendPhoneRegistrationOTP, verifyPhoneOTP, initiateEmailVerificationOTP, verifyEmailOTPHandler, resendEmailOTPHandler, completeRegistration, requestLoginOTP, resendLoginOTP, loginWithPhone, loginWithEmail, logout, logoutAll, refreshAccessToken, listSessions, revokeSession, forgotPassword, resetPassword, getCurrentUser, updateUserProfile, } from "../controllers/user.controller.js";
import { authenticateUser } from "../middlewares/auth.middleware.js";
import { authLimiter, otpLimiter, passwordResetLimiter, } from "../middlewares/rateLimiter.middleware.js";
const userRouter = router.Router();
// ─── Registration ─────────────────────────────────────────────────────────────
userRouter.post("/auth/register/initiate-phone", otpLimiter, initiatePhoneRegistration);
userRouter.post("/auth/register/resend-phone-otp", otpLimiter, resendPhoneRegistrationOTP);
userRouter.post("/auth/register/verify-phone", otpLimiter, verifyPhoneOTP);
userRouter.post("/auth/register/initiate-email-otp", otpLimiter, initiateEmailVerificationOTP);
userRouter.post("/auth/register/verify-email-otp", otpLimiter, verifyEmailOTPHandler);
userRouter.post("/auth/register/resend-email-otp", otpLimiter, resendEmailOTPHandler);
