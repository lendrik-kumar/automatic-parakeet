...
const calculateSummary = async (cart, region) => {
    ...
    const shippingRule = await shippingRuleRepository.findMatchingRule(taxableAmount, region ?? null);
...
const ensureValidAndAvailableVariant = async (variantId, neededQty) => {
...
export const getCart = async (userId, region) => {
...
export const addItem = async (userId, data) => {
    ...
    const { variant, inventory } = await ensureValidAndAvailableVariant(data.variantId, data.quantity);
    ...
    if (existing) {
        ...
        await cartRepository.updateItemQuantity(existing.id, newQty, round2(newQty * variant.price));
...
export const updateItemQuantity = async (userId, itemId, quantity) => {
    ...
    await cartRepository.updateItemQuantity(itemId, quantity, round2(quantity * item.unitPrice));
    await cartRepository.updateCartTotals(cart.id);
    return cartRepository.findByCustomerId(userId);
};
...
export const bulkAddItems = async (userId, items) => {
    const cart = await cartRepository.findByCustomerId(userId);
...
export const bulkUpdateItems = async (userId, updates) => {
    const cart = await cartRepository.findByCustomerId(userId);
    if (!cart)
        throw new CartError(404, "Cart not found");
    ...
    for (const update of updates) {
        ...
        if (diff > 0) {
            ...
            if (!inventory || inventory.availableStock < diff) {
                throw new CartError(400, `Insufficient stock for item ${existing.variant.sku}`);
            }
        }
    }
    for (const update of updates) {
        ...
        await cartRepository.updateItemQuantity(existing.id, update.quantity, round2(update.quantity * existing.unitPrice));
    }
    await cartRepository.updateCartTotals(cart.id);
    return cartRepository.findByCustomerId(userId);
};
/** DELETE /cart/items/bulk */
export const bulkRemoveItems = async (userId, itemIds) => {
    const cart = await cartRepository.findByCustomerId(userId);
    if (!cart)
        throw new CartError(404, "Cart not found");
...
export const applyCoupon = async (userId, couponCode) => {
    ...
    const subtotal = round2(cart.items.reduce((sum, item) => sum + item.totalPrice, 0));
    if (subtotal <= 0)
        throw new CartError(400, "Cannot apply coupon to empty cart");
    let validation;
    try {
    ...
    if (!coupon)
        throw new CartError(404, "Coupon not found");
...
/** GET /cart/summary */
export const getCartSummary = async (userId, region) => {
    const cart = await cartRepository.findByCustomerId(userId);
    if (!cart) {
        return {
            items: [],
...
export const moveItemToWishlist = async (userId, itemId) => {
    const cart = await cartRepository.findByCustomerId(userId);
    if (!cart)
        throw new CartError(404, "Cart not found");
    const item = await cartRepository.findItemById(itemId);
    ...
    const existingWishlistItem = await wishlistRepository.findExistingItem(wishlist.id, item.productId, item.variantId);
...
export const addItemFromWishlist = async (userId, wishlistItemId) => {
    const wishlistItem = await wishlistRepository.findItemById(wishlistItemId);
    if (!wishlistItem)
        throw new CartError(404, "Wishlist item not found");
...
export const saveItemForLater = async (userId, itemId) => {
    const item = await cartRepository.findItemById(itemId);
    if (!item)
        throw new CartError(404, "Cart item not found");
    const cart = await cartRepository.findByCustomerId(userId);
...
export const restoreSavedItem = async (userId, savedItemId) => {
    const savedItem = await savedItemRepository.findById(savedItemId);
    if (!savedItem)
        throw new CartError(404, "Saved item not found");
...
/** POST /cart/sync */
export const syncCart = async (userId, payload) => {
    ...
    const byKey = (item) => `${item.variantId}:${item.size}:${item.color}`;
    const serverMap = new Map(currentCart.items.map((item) => [
    ...
    for (const localItem of payload.items) {
        ...
        const targetQty = strategy === "KEEP_HIGHER"
    ...
    if (!syncedCart)
        throw new CartError(500, "Failed to sync cart");
    return syncedCart;
};
