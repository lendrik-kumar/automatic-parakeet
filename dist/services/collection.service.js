...
export const createCollection = async (adminId, data) => {
    ...
    await adminRepository.logActivity(adminId, "CREATE", "Collection", collection.id);
    return collection;
};
export const updateCollection = async (adminId, id, data) => {
...
export const deleteCollection = async (adminId, id) => {
    const existing = await collectionRepository.findById(id);
    if (!existing)
        throw new CollectionError(404, "Collection not found");
    ...
    if (productCount > 0) {
        throw new CollectionError(400, `Cannot delete collection with ${productCount} products. Please reassign or delete products first.`);
    }
    await collectionRepository.hardDelete(id);
    await adminRepository.logActivity(adminId, "DELETE", "Collection", id);
};
export const updateCollectionStatus = async (adminId, id, status) => {
    const existing = await collectionRepository.findById(id);
    if (!existing)
        throw new CollectionError(404, "Collection not found");
...
export const reorderCollections = async (adminId, orderMap) => {
    // Validate all collections exist
    const collections = await Promise.all(orderMap.map((item) => collectionRepository.findById(item.id)));
    ...
    await adminRepository.logActivity(adminId, "UPDATE", "Collection", "bulk-reorder");
};
export const getCollectionProducts = async (id, page = 1, limit = 20) => {
    const collection = await collectionRepository.findById(id);
    if (!collection)
        throw new CollectionError(404, "Collection not found");
    const skip = (page - 1) * limit;
...
export const getCollectionStats = async (id) => {
    const collection = await collectionRepository.findById(id);
    if (!collection)
        throw new CollectionError(404, "Collection not found");
