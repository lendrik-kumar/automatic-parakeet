...
import { checkEventStatus, acquireProcessingLock, releaseProcessingLock, markEventAsProcessed, } from "../utils/webhookEventTracker.js";
import { createRazorpayOrder, fetchRazorpayPayment, getRazorpayKeyId, } from "../lib/razorpay.js";
import { convertToSmallestUnit, verifyPaymentSignature, generateOrderReceipt, parseRazorpayError, isValidRazorpayOrderId, isValidRazorpayPaymentId, } from "../utils/razorpayHelpers.js";
 ...
 */
export const createRazorpayPaymentSession = async (input) => {
    try {
        ...
        if (!payment) {
            ...
            if (existingPayment) {
                ...
                payment = await paymentRepository.updateRazorpayDetails(existingPayment.id, {
                    razorpayOrderId: razorpayOrder.id,
                    paymentGatewayResponse: razorpayOrder,
                });
            }
            else {
...
export const verifyRazorpayPayment = async (input) => {
    try {
        ...
        if (payment.paymentStatus === "COMPLETED") {
            console.log("[PaymentService] Payment already completed (idempotent):", payment.id);
            return payment;
        }
        // Verify signature
        const isValid = verifyPaymentSignature(input, process.env.RAZORPAY_KEY_SECRET);
        if (!isValid) {
 ...
 */
const handlePaymentSuccess = async (webhookPayload, event) => {
    const razorpayPayment = webhookPayload.payment.entity;
 ...
 */
const handlePaymentFailure = async (webhookPayload, event) => {
    const razorpayPayment = webhookPayload.payment.entity;
    const razorpayOrderId = razorpayPayment.order_id;
    // Find payment by Razorpay order ID
    const payment = await paymentRepository.findByRazorpayOrderId(razorpayOrderId);
