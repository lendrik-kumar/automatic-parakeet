...
export const createCoupon = async (adminId, data) => {
    ...
    if (existing)
        throw new CouponError(409, "Coupon code already exists");
...
export const getCoupon = async (couponId) => {
    ...
    if (!coupon)
        throw new CouponError(404, "Coupon not found");
    return coupon;
};
export const updateCoupon = async (adminId, couponId, data) => {
    ...
    const coupon = await couponRepository.update(couponId, updateData);
    await adminRepository.logActivity(adminId, "UPDATE", "Coupon", couponId);
    return coupon;
};
export const deleteCoupon = async (adminId, couponId) => {
    const existing = await couponRepository.findById(couponId);
    if (!existing)
        throw new CouponError(404, "Coupon not found");
...
export const updateCouponStatus = async (adminId, couponId, status) => {
    const existing = await couponRepository.findById(couponId);
    if (!existing)
        throw new CouponError(404, "Coupon not found");
...
export const validateCoupon = async (code, orderTotal) => {
    ...
    if (coupon.minimumOrderValue && orderTotal < coupon.minimumOrderValue) {
        throw new CouponError(400, `Minimum order value is $${coupon.minimumOrderValue}`);
    }
