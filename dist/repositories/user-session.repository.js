...
export const userSessionRepository = {
    ...
    create: (data) => prisma.userSession.create({
        data: {
            userId: data.userId,
    ...
    findByRefreshToken: (refreshToken) => prisma.userSession.findFirst({
        where: { refreshToken },
        include: { user: true },
    }),
    /** Find a session by id scoped to a user */
    findByIdAndUser: (id, userId) => prisma.userSession.findFirst({ where: { id, userId } }),
    /** List all sessions for a user */
    listByUser: (userId) => prisma.userSession.findMany({
        where: { userId },
        select: {
            id: true,
            device: true,
    ...
    delete: (id) => prisma.userSession.delete({ where: { id } }),
    ...
    deleteAllByUser: (userId) => prisma.userSession.deleteMany({ where: { userId } }),
};
